...
import { NAMESPACE } from './transaction';

// Make a unique hamlet address
const NS_HASH = x => crypto.createHash('sha512').update(x).digest('hex').toLowerCase()
  .substring(0, 64);
...
const spaceContains = (num, space) => space.START <= num < space.STOP;
...
function addressIs(address) {
  ...
  const infix = parseInt(address.slice(6, 8), 16);
  console.log(infix);
